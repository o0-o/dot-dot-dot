...
# otherwise use $XDG_HOME_DATA/xdg
[ -d '/etc/xdg' ]                                                 &&
...
# bin
typeset XDG_BIN_DIR="${HOME}/.local/bin"                          &&
{ [ -d "${XDG_BIN_DIR}" ] ||
  install -m 700 -d \
    "${XDG_BIN_DIR}"
}                                                                 &&
...
{ printf '%s' "${PATH}" |
    egrep -q "${XDG_BIN_DIR}:|:${XDG_BIN_DIR}"  ||
  typeset PATH="${XDG_BIN_DIR}:${PATH}"
}                                                                 &&
...
for COMPAT_BIN_DIR in "${HOME}/bin" "${HOME}/Applications"; do
  ...
  # stop if $XDG_BIN_DIR is already equivalent $COMPAT_BIN_DIR
  [[ ! "$(cd "${XDG_BIN_DIR}"; pwd)" == "$(cd "${COMPAT_BIN_DIR}"; pwd)" ]] &&

  # replace $COMPAT_BIN_DIR with symlink to $XDG_BIN_DIR
  rm -r "${COMPAT_BIN_DIR}"                                                 &&
  ln -s "${XDG_BIN_DIR}" \
        "${COMPAT_BIN_DIR}"                                                 ||
...
# enforce xdg
typeset LESSHISTFILE="${XDG_DATA_DIR:-$HOME/.local}/less/lesshst" &&
install -m '700' -d \
  "${LESSHISTFILE%/*}"                                            &&
